...
public class TrainWidgetProvider extends AppWidgetProvider {
    ...
    public TrainWidgetProvider() {
        ...
        HandlerThread sWorkerThread = new HandlerThread("TrainWidgetProvider-worker");
        sWorkerThread.start();
    ...
    public void onEnabled(Context context) {
        ...
        if (sDataObserver == null) {
            ...
            final ComponentName cn = new ComponentName(context, TrainWidgetProvider.class);
            sDataObserver = new TrainDataProviderObserver(mgr, cn, sWorkerQueue);
            r.registerContentObserver(TrainDataProvider.CONTENT_URI, true, sDataObserver);
        }
    }
        ...
        if (action.equals(REFRESH_ACTION) || action.equals(UPDATE_ACTION)) {
            ...
            sWorkerQueue.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mSTOPCursor.getCount() > 1) {
                        ...
                        if (action.equals(REFRESH_ACTION)) {
                            ...
                            if (v.getVehicleStops().getVehicleStop().size() > 1) {
                                ...
                                updateTime = Utils
                                        .formatDateWidget(new Date());
                                mDbHelper.createWidgetStop(tid, "1", updateTime, fromTo);
                                for (UtilsWeb.VehicleStop stop : v.getVehicleStops().getVehicleStop()) {
                                    mDbHelper.createWidgetStop(stop.getStation(), "" + stop.getTime(), stop.getDelay(), stop.getStatus());
                                }
                            }
                        }
                    ...
                    final Cursor c = r.query(TrainDataProvider.CONTENT_URI, null, null, null,
                            null);
                    ...
                    for (int i = 0; i < count; ++i) {
                        final Uri uri = ContentUris.withAppendedId(TrainDataProvider.CONTENT_URI, i);
                    ...
                    final ComponentName cn = new ComponentName(ctx, TrainWidgetProvider.class);
                    ...
                    final RemoteViews rv = new RemoteViews(ctx.getPackageName(), R.layout.widget_layout);

                    if (tid != null) {
                        detailIntent = new Intent(ctx, InfoTrainActivity.class);
                        detailIntent.putExtra("fromto", fromTo);
                        detailIntent.putExtra("Name", tid);
                        pendingIntent = PendingIntent.getActivity(ctx, 0,
